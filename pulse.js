	...
	startServer: function(callback) {
		...
		exec("pactl stat", function (error, stdout, stderr) {
			...
			exec("pactl load-module module-null-sink sink_name='GrabBoard' sink_properties=device.description='Sound_grab_Null_sink'", function (error, stdout, stderr) {
				...
				exec("pactl load-module module-null-sink sink_name='PlaybackBoard' sink_properties=device.description='Playback_grab_Null_sink'", function (error, stdout, stderr) {
					if (error) {
						printStatus(2)
						process.exit(0)
					}
					...
					exec("pactl load-module module-loopback source='GrabBoard.monitor' sink='PlaybackBoard'", function (error, stdout, stderr) {
						if (error) {
							printStatus(2)
							process.exit(0)
						}
						...
						exec("pactl load-module module-loopback source='GrabBoard.monitor' sink='" + defaultSink + "'", function (error, stdout, stderr) {
							...
							exec("pactl load-module module-loopback source='" + defaultSource + "' sink='PlaybackBoard'", function (error, stdout, stderr) {
								if (error) {
									printStatus(2)
									process.exit(0)
								}
	...
	bindRecording: function(sinkId, callback) {
		...
		exec("pactl list short sources", function (error, stdout, stderr) {
			...
			for (var i = 0; i < sources.length; i++) {
				if (sources[i].indexOf("PlaybackBoard.monitor") > -1) {
